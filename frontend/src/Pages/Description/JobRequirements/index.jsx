...
const JobReq = () => {
  return (
    <div class='custom-bg-about p-6 m-4'>
        <div class='text-2xl text-gray-800 custom-col-blue p-2'>Job Requirements</div>
          ...
          {jobRequirement.map((item, index) => (
            <div key={index} className="grid-item flex gap-4">
              ...
              <div class='flex justify-center items-center'>
                <input type="radio" className="h-6 w-6 text-blue-500 border-blue-500"/>
              </div>
              <div class='text-xl text-gray-800 flex justify-center items-center'>{item.title}</div>
            </div>
          ))}
        </div>
    </div>
